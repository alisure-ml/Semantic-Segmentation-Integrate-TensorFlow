...
import DeepLabV3_resnet_utils as resnet_utils
from DeepLabV3_resnet_v1 import bottleneck, resnet_arg_scope
...
def deeplabv3(inputs, num_classes, depth=50, aspp=True, reuse=None, is_training=True,
              weight_decay=1e-4, bn_weight_decay=0.9997):
    """DeepLabV3
    Args:
      inputs: A tensor of size [batch, height, width, channels].
    ...
    with tf.variable_scope('resnet{}'.format(depth), [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope(resnet_arg_scope(weight_decay=weight_decay, batch_norm_decay=bn_weight_decay)):
